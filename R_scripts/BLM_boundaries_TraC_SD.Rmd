...
Date: 11 April 2023
File name for script: BLM_boundaries_TraC_SD.Rmd
...
Add the name of the SE variable as in the data set used, in this example *SecchiDepth_m*  
...
# Select the data for SE Transparency (SD)
datIC.SD <- datIC %>% 
...
out <- modBLM(df=datIC.SD.BC4 ,plt = FALSE, x=SE,y=BiotaClass,min_new.x=1,max_new.x=1,logarithm=TRUE)
...
df <- datIC.SD.BC4  #select data to plot
...
pGLM_z(df=datIC.SD.NEA,x=SE,z="ICtype") #EDIT z= "" as needed
```
...
```{r}
# set z to any categorical variable name in the data set
pGLM_z_Alls(df=datIC.SD.NEA,x=SE,z="ICtype") # function with #EDIT z= "" as needed
```
