...
Some P data seem to have has been reported in the wrong units (ug/L) rather than the stated (mg/L). But not sure - check with old reports and with MS.
Focus on the paramenters we discussed for use in the approach testing report: 
TN, NO3, Secchi Depth (SD) and Dissolved Oxygen (DO).
```{r WQ parameters check plots}
#TW
...
First dataset - importing the WFD data for 2nd reporting cycle 2016 (only EQS categorical data at WB level).
```{r Import WFD WISEv4 Biota EQS classifications}
#TW
dat_WFD_TW <- readRDS(here("Data","dat_WFD_TW.rds")) #import biology

dat_WFD_TW_2nd <- dat_WFD_TW %>% #select only 2nd cycle assessments 2016
  filter(cYear==2016) %>% 
  filter(StartYear > 2009 |is.na(StartYear)) %>% #n=1423 after filter applied
  ungroup()
...
#CW
dat_WFD_CW <- readRDS(here("Data","dat_WFD_CW.rds")) #import biology
...
Second dataset - importing the SOE WISE2 biology data (both nEQRs and EQS categorical data). Data available at site (disagg) and WB level(agg).
```{r Import WISE2 Biology nEQR & EQS classifications}
#TW
...
WFD EQS data WB level 2016.
```{r Combine WQ and WFD EQS classifications}
#merge WFD Biology and SE for TW 
...
notes <- "2nd cycle (2016) WFD classification (WFD WISEv4) linked by WB to WISE6 SoE WQ data (2010-2015) averaged by WB for TW. Created in CombineTraC_SE-BQE.Rmd"
...
notes <- "2nd cycle (2016) WFD classification (WFD WISEv4) linked by WB to WISE6 SoE WQ data (2010-2015) averaged by WB for CW. Created in CombineTraC_SE-BQE.Rmd"
