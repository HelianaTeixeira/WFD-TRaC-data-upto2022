...
```{r WQ data summarised per WB-Cycle wide format}
...
#loop through them and store results in a named list:
#TW
...
for (p in names(periods)) {
  ...
  DetsTW.list_Period[[p]] <- levels(factor( WQdatTWSite_Period[[p]]$Det)) # list available dets
...
for (p in names(periods)) {
 ...
 #filter to years in the period
  ...
  #extract available determinands for this period
  DetsCW.list_Period[[p]] <- levels(factor( WQdatCWSite_Period[[p]]$Det)) # list available dets
...
head(WQ_CW_WBsummary_Period$`2010_2015`)
 ...
 {WQ_CW_WBsummary_Period$`>2022`} |>
  dplyr::summarise(n = dplyr::n(), .by = c(countryCode, waterBodyIdentifier, waterBodyName, nYears, Det)) |>
  dplyr::filter(n > 1L)

```

```{r WQ data WB-cycle combined wide Table}
#TW
WQ_TWWB_master <- bind_rows(
  lapply(names(WQ_TWWB_wide), function(p) {
    WQ_TWWB_wide[[p]] %>% mutate(Period = p)
  })
) %>% 
  relocate(Period, .before = 1)   # put Period as first column

head(WQ_TWWB_master) #check

WQ_CWWB_master <- bind_rows(
  lapply(names(WQ_CWWB_wide), function(p) {
    WQ_CWWB_wide[[p]] %>% mutate(Period = p)
  })
) %>% 
  relocate(Period, .before = 1)   # put Period as first column

head(WQ_CWWB_master) #check

```

```{r WQ data change units }
...
for (p in names(WQ_TWWB_wide)) {
  addWorksheet(wb, sheetName = p)
  writeData(wb, sheet = p, x = WQ_TWWB_wide[[p]])
}

saveWorkbook(wb, "WQ_TWWBsummary_Periods.xlsx", overwrite = TRUE)

#CW 
wbCWWB <- createWorkbook()

for (p in names(WQ_CWWB_wide)) {
  addWorksheet(wb, sheetName = p)
  writeData(wb, sheet = p, x = WQ_CWWB_wide[[p]])
}

saveWorkbook(wb, "WQ_CWWBsummary_Periods.xlsx", overwrite = TRUE)
