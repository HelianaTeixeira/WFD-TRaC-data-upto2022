...
```{r overview of reporting period annual / seasonal}
#check parameterSamplingPeriod values
#LEFT HERE
#TW only Agg data has sampling period check at origin extract data script
# ALSO empty info (NAs) to consider first
# and ALSO different seasons reported
#adjust code below or change mutated new variable name as needed
WQdatTWSiteAgg <- WQdatTWSiteAgg %>% 
  mutate(sampling = ifelse(
    grepl("^[0-9]{4}-01--[0-9]{4}-12$|^[0-9]{4}-[0-9]{2}-01--[0-9]{4}-12-31$",
          WQdatTWSiteAgg$parameterSamplingPeriod),
  "annual data",
  "seasonal data"))
