...
Date:3 March 2023
File name for script: BLM_boundaries_TraC.Rmd
...
All of the functions used are in file *Function_BLM.R* which  needs to be loaded


##  Setup

First we load the libraries needed and run the user functions in R script *Functions_BLM.R*
```{r, include=TRUE}
...
Add the name of the SE variable as in the data set used, in this example *Secchi.depth*  

Add the text used for axis labels in plot functions  

Add the name of the binary dependent variable, the biological class (1 = Good, 0 =Not Good). The variable name *BinClass* is generated in this script from the NEQR. However, if classification data are used, for example the WFD data sets then a different name can be used  
...
SE <- "SecchiDepth_m"    # name of data column containing SE in data file        
xlabel="transparency (m)"
...
# Get the data
  #Lake SoE data (WB, Year)
  #datSoE <- readRDS(file = "O:/OneDrive/Work/Projects/2021/JRC/NutBound/Data/SoE_data/dat3L.bio.wq.rds")
datIC <- read.csv(file = here("Databases","IC_selected_datasets.csv"),sep = ",")

# Select the Type-SE-BQE data
datIC.mac <- datIC %>% 
  select (BQE,Country,WaterCategory,GIG,ICtype,Date_BQE,EQR,EQS,SecchiDepth_m)%>%
  filter(BQE=="Macroalgae") %>%
...
dat.IC.mac.BC4 <- datIC.mac %>% 
  filter(BQE %in% c("Macroalgae")) %>%  #list options (e.g. types) to select #Geoff: CodeAggr & "type code"
  filter(!is.na(.data[[SE]])) #remove missing values

#EDIT countries biota EQR G/M boundary when not NEQR
IC.GM_MS1 <- 0.7 # EDIT: MS IC EQS boundary if not using normalised EQR
IC.GM_MS2 <- 0.75 # add more MS if needed (and reflect in "Functions_BLM_EQR.R" to get all hlines in Fig1d)

#EDIT: Run model and calculate measures, remember to edit line to  match dataframe name created in line above!
out <- modBLM(df=dat.IC.mac.BC4 ,plt = FALSE, x=SE,y=BiotaClass,min_new.x=1,max_new.x=1,logarithm=TRUE)
...
df <- dat.IC.mac.BC4  #select data to plot
...
#meas_u <- "OmisComm"  #To plot a different measure use this line

tsize=2  #  modify the size of text

p1 <- GetFig1a(df=df,meas=meas_u,colName = SE,EQRVar="NEQR",adj=0,dec=1,negResp = TRUE,jith=0)
p2 <- GetFig1b(df=df,meas=meas_u,colName = SE,negResp = TRUE) #EDIT: "TRUE" or "FALSE"
p3 <- GetFig1c(df=df,meas=meas_u,colName= SE)
p4 <- GetFig1d(df=df,meas=meas_u,EQRVar="NEQR",colName= SE,negResp=TRUE)
...
IC.GM_MS2 <- 0.645 # add more MS if needed (and reflect in "Functions_BLM_EQR.R" to get all hlines in Fig1d)

#LEFT HERE
dat.IC.mac2.NEA11 <- dat.IC.mac2.NEA11 %>%
...
out <- modBLM(df=dat.IC.mac2.NEA11 ,plt = FALSE, x=SE,y=BiotaClass,min_new.x=1,max_new.x=1,logarithm=TRUE)
...
df <- dat.IC.mac2.NEA11  #select data to plot
meas_u <- "OmisComm"  #select measure to use

tsize=2  #  modify the size of text
p7 <- GetFig1a(df=df,meas="Com0.1",colName = SE,EQRVar="NEQR",adj=0,dec=2,negResp = TRUE,jith = 0.01)
p8 <- GetFig1b(df=df,meas="TKitMM",colName = SE,negResp = TRUE)
p9 <- GetFig1c(df=df,meas="TKitMM",colName= SE)
p10 <- GetFig1d(df=df,meas="TKitMM",EQRVar="NEQR",colName= SE,negResp=TRUE)
...
pGLM_z(df=dat.IC.mac2.NEA11,x=SE,z="countryCode") #EDIT z= "" as needed
```
...
```{r}
# set z to any categorical variable name in the data set
pGLM_z_Alls(df=dat.IC.mac2.NEA11,x=SE,z="countryCode") # function with #EDIT z= "" as needed
```
